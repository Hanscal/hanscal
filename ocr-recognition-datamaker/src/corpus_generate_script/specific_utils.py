...
from charset_random import cn_words
...
def get_random_company():
    global company_line_data
    n = len(company_line_data)
    if n == 0:
        company_line_data = load_company_line_data(os.path.join(base_data_dir, 'company.txt'))
        n = len(company_line_data)
...
def get_random_person_name():
    global chinese_name_line_data
    n = len(chinese_name_line_data)
    if n == 0:
        chinese_name_line_data = load_company_line_data(os.path.join(base_data_dir, 'chinese_name_data.txt'))
        n = len(chinese_name_line_data)
...
def get_random_street():
    global street_line_data
    n = len(street_line_data)
    if n == 0:
        street_line_data = load_company_line_data(os.path.join(base_data_dir, 'gen_streets.txt'))
        n = len(street_line_data)
